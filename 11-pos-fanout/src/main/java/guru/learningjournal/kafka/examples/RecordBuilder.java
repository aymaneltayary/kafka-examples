...
class RecordBuilder {
    ...
    static List<HadoopRecord> getHadoopRecords(PosInvoice invoice) {
        ...
        for (LineItem i : invoice.getInvoiceLineItems()) {
            HadoopRecord record = new HadoopRecord()
                    ...
                    .withCreatedTime(invoice.getCreatedTime().toString())
            ...
            if (invoice.getDeliveryType().equalsIgnoreCase(AppConfig.DELIVERY_TYPE_HOME_DELIVERY)) {
    ...
    static Notification getNotification(PosInvoice invoice) {
        return new Notification()
                ...
                .withEarnedLoyaltyPoints(invoice.getTotalAmount() * AppConfig.LOYALTY_FACTOR);
    }
}
